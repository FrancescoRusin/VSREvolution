            ...
            for (int name = 0; name < 3; name++) {
                listener = ListenerUtils.build(
                        "best,last,all", "best,last,all", Map.ofEntries(
                                Map.entry("best", "best_" + names[name] + "_" + counter),
                                Map.entry("last", "last_" + names[name] + "_" + counter),
                                Map.entry("all", "all_" + names[name] + "_" + counter)
                        ), Map.ofEntries(
                                Map.entry("experiment.name", "distance_run"), Map.entry("sensor.config", "t+a+vxy"),
                                Map.entry("solver", "numGA,nPop=100,nEval=10000")
                        ));
                distanceRuns = bodies[name].distanceRun(1, 1, true,
                        100, 10000, 10,
                        "best_robots_" + names[name] + "_" + counter + ".csv", listener);
                distanceRunsString = "";
